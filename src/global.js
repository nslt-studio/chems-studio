...
let onInquiryOpenClick = null;
let onInquiryCloseClick = null;
let onInquiryKeydown = null;

function openInquiry() {
  const panel = document.querySelector("#inquiry");
  if (!panel || inquiryOpen) return;
  inquiryOpen = true;
  panel.style.opacity = "1";
  panel.style.pointerEvents = "auto";
  document.body.style.overflow = "hidden";

  panel.querySelectorAll("[scramble]").forEach((el) => {
...
function initInquiry() {
  onInquiryOpenClick = () => openInquiry();
  onInquiryCloseClick = () => closeInquiry();
  onInquiryKeydown = (e) => { if (e.key === "Escape") closeInquiry(); };

  document.querySelectorAll("[data-button='inquiry']").forEach((btn) => {
    btn.addEventListener("click", onInquiryOpenClick);
  });

  const closeBtn = document.querySelector("#inquiryClose");
  if (closeBtn) closeBtn.addEventListener("click", onInquiryCloseClick);

  window.addEventListener("keydown", onInquiryKeydown);
}

function destroyInquiry() {
  document.querySelectorAll("[data-button='inquiry']").forEach((btn) => {
    btn.removeEventListener("click", onInquiryOpenClick);
  });

  const closeBtn = document.querySelector("#inquiryClose");
  if (closeBtn) closeBtn.removeEventListener("click", onInquiryCloseClick);

  window.removeEventListener("keydown", onInquiryKeydown);

  onInquiryOpenClick = null;
  onInquiryCloseClick = null;
  onInquiryKeydown = null;
  closeInquiry();
}
