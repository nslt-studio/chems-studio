...
function updateVideoTimecode() {
  ...
  if (totalEl) {
    ...
    if (isVideo) {
      ...
      if (overlay) overlay.style.clipPath = `inset(0 ${100 - progress}% 0 0)`;
...
function initInfo() {
  ...
  onInfoClick = () => {
    ...
    const label = infoOpen ? "Hide" : "Credits";
    infoBtn.textContent = "";
    scrambleText(infoBtn, label);
...
function initSlider() {
  ...
  embla = EmblaCarousel(viewport, { loop: true, axis: "x" });
